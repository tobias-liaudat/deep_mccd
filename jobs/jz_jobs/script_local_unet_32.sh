#!/bin/bash
#SBATCH --job-name=local_unet_32    # nom du job
...
#SBATCH --output=local_unet_32_%j.out  # nom du fichier de sortie
#SBATCH --error=local_unet_32_%j.err   # nom du fichier d'erreur (ici commun avec la sortie)
...
srun python -u training_unets.py \
    --run_id_name local_unet_32 \
    --dataset_path /gpfswork/rech/ynx/ulx23va/deep_mccd_project/eigenpsf_datasets/local_eigenpsfs.fits \
    --base_save_path /gpfswork/rech/ynx/ulx23va/deep_mccd_project/trained_models/local_unet_32/ \
    --layers_n_channel 32 \
    --layers_levels 5 \
    --kernel_size 3 \
