#!/bin/bash
#SBATCH --job-name=all_unet_64    # nom du job
...
#SBATCH --output=all_unet_64_%j.out  # nom du fichier de sortie
#SBATCH --error=all_unet_64_%j.err   # nom du fichier d'erreur (ici commun avec la sortie)
...
srun python -u training_unets.py \
    --run_id_name all_unet_64 \
    --dataset_path /gpfswork/rech/ynx/ulx23va/deep_mccd_project/eigenpsf_datasets/all_eigenpsfs.fits \
    --base_save_path /gpfswork/rech/ynx/ulx23va/deep_mccd_project/trained_models/all_unet_64/ \
    --layers_n_channel 64 \
    --layers_levels 5 \
    --kernel_size 3 \
